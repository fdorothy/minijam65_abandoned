...
public class RoomGenerator : MonoBehaviour
{
    ...
    Dictionary<Vector2, DungeonRoom> MainRooms;
    ...
    Dictionary<DungeonRoom, int> ConnectionCounter = new Dictionary<DungeonRoom, int>();
    ...
    public void Generate(int roomCount, int radius, float mainRoomFrequency, float roomConnectionFrequency)
    {
        ...
        {
            DungeonRoom room = (GameObject.Instantiate(Resources.Load("Room") as GameObject)).GetComponent<DungeonRoom>();
    ...
    private void ProcessRoomConnections(bool drawLines)
    {
        ...
        for (int n = 0; n < Rooms.Count; n++)
        {
            if (Rooms[n].IsMainRoom)
            {
                ...
                {
                    DungeonRoom connectingRoom = Rooms[n].Connections[i].Room;
                    ...
                    {
                        hit[x].collider.GetComponent<DungeonRoom>().SetVisible(true);
                    }
    ...
    private void SetStartAndEndRooms()
    {
        ...
        List<DungeonRoom> roomsWithOneConnection = new List<DungeonRoom>();
        List<DungeonRoom> roomsWithTwoConnection = new List<DungeonRoom>();

        //check connection counters
        foreach (KeyValuePair<DungeonRoom, int> kvp in ConnectionCounter)
        {
            if (kvp.Value == 1)
            {
    ...
    void Calculate()
    {
        ...
        MainRooms = new Dictionary<Vector2, DungeonRoom>();
    ...
    public Vector3 ToWorld(Vector2 pt) => new Vector3(pt.x - XMin, 0.0f, pt.y - YMin);
}
